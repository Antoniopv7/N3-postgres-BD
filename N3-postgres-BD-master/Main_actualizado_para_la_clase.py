...
def consultar_id(self):
    ...
    obtener_id = self.id_cliente.get()
    try:
        conn = self.abrir()
        cursor = conn.cursor()
        cursor.execute("Select cliente_id from boleta where cliente_id = '%s'"%(obtener_id))
        r = cursor.fetchall()
    finally:
        conn.close()
    if len(r)>0:
...
def mostrar_productos(x): ##LISTADO PRODUCTOS Y CONTROL DE STOCK
    ...
    cursor=conn.cursor()
    #query= 'SELECT producto_id, stock, CASE when stock >= 10 then '' when stock < 5 then 'Rabastecer' else '' END Control_Stock FROM producto;'
    cursor.execute(query)
...
def ventana7():
    ...
    label1 = Label(v6,text="| Id_producto || Nombre |").grid(row=3,column=1)
...
def ventana18():
    ...
    label = Label(v3,text="|Id Cliente|").grid(row=3,column=1)
...
def ventana19():
    ...
    label = Label(v3,text="|Id Empleado|").grid(row=3,column=1)
...
def ventana20():
    ...
    label = Label(v3,text="|Id Despacho|").grid(row=3,column=1)
...
def ventana21():
    ...
    label = Label(v3,text="|Código Pago|").grid(row=3,column=1)
...
def ventana22():
    ...
    label = Label(v3,text="|Id Producto|").grid(row=3,column=1)
...
def ventana23():
    ...
    label = Label(v3,text="|Id Proveedor|").grid(row=3,column=1)
...
def ventana24():
    ...
    label = Label(v3,text="|Código Pedido|").grid(row=3,column=1)
...
def ventana_m1():
    ...
    label = Label(v3,text="|Id Cliente|").grid(row=8,column=1)
    mostrar_id_cliente(v3)
...
def ventana_m5():
    ...
    label = Label(v3,text="|Id Producto|").grid(row=8,column=1)
    mostrar_id_producto2(v3)
